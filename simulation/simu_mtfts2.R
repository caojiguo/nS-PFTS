...
functTSTrend1 <- function (N=100, m=50, norm.op =0.5,theta0=12,sigma=1, periodM,burn =50) {
  # N= sample size
  # m= num of points in [0 ,1]
  h=1/m
  s=seq (1,m, by=1)/m
...
simulation1<-function(N,m,N_sim, norm.op,theta0,K1,K2,K3,c_a,sigma,periodM){
  ...
  for (j in 1:N_sim){
    Data <- functTSTrend1 (N,m, norm.op,theta0,sigma,periodM)
    YY <- Data$DataM
    #
    ...
    k<-1
    for(thetai in pseq){
      Q[k]=Q2[k]+c_a*(m*N)^(1/6)*log(N)*(thetai)*s1*m*0.9
      k=k+1
    }
...
set.seed (1985)
gammi=3*rnorm(theta0*K3,0,1)
#gamm=matrix(gammi,K3,theta0)
periodM1=basismatx %*%gammi
periodM=matrix(periodM1,m,N)

s_time=proc.time()
result0<-simulation1(N,m,N_sim, norm.op,theta0,K1,K2,K3,c_a,sigma,periodM)
e_time=proc.time()
